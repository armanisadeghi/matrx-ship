...
- **Closed 2026-07-07:** ~~generic config-file editor~~ → **`/admin/files`** (file-tree browse/view/edit on the /srv host and EC2 fleet hosts over SSM, `.bak` on every save, superadmin). ~~EC2 env editing~~ → **Secrets remote stores** for active EC2 services, including `ec2:sandbox-orchestrator`, with a per-store **"Apply (restart)"** button (compose stores force-recreate; SSM stores restart the service with a patient health probe; the Manager's own store deliberately has no Apply). The retired AI Dream EC2 replica and its `ec2:aidream-app` store were removed 2026-08-20. Fleet Health mirrors actionable findings into aidream ops-triage and checks canonical ECS AI Dream health plus both sandbox tiers.
