...
| **AWS production topology + live migration ledger** | **[common-docs production infrastructure](../common-docs/systems/production-infrastructure/FEATURE.md) — ECS/Fargate, Supabase east migration, scraper boundary, cutover and rollback gates** |
| **Generate and host customer MCPs on Cloudflare, Coolify, or AWS** | **[generated MCP hosting](../common-docs/systems/mcp-hosting/FEATURE.md) — target selection, App Runner workload identity, registry, and agent workflow** |
...
- This repo's own `.matrx.json` points at `https://matrx-ship.dev.codematrx.com` — **the platform ships itself** through its own pipeline. Since 2026-07-07 that pipeline is **push-to-main → CI builds GHCR images → a 2-min host poller (`matrx-ship-deploy.timer` → [scripts/pull-deploy.sh](scripts/pull-deploy.sh)) deploys them** (Manager health-gated + rollback). The GHA SSH deploy job is best-effort only.
...
- **GITHUB_PAT expiry = simultaneous 401s across fleet checks (deploys / aidream-pipeline / hosted-deploy).** Happened 2026-07-09: the PAT expired 2 days after creation and three checks went unknown/warning at once. That pattern IS the diagnosis — replace `GITHUB_PAT` via the Secrets page (Manager store) with a **no-expiry** PAT that can read armanisadeghi/matrx-sandbox + AI-Matrix-Engine/aidream, then recreate the Manager (push-to-main rolls it, or the Deploy server).
- **Fleet Health monitor (2026-05-25):** read-only `GET /api/fleet-health` + `/admin/fleet-health` page + a global red banner. Catches the failures that used to be silent — **cross-host code/config drift** (hosted vs EC2 orchestrator), **stale/missing sandbox images**, and **failed deploy runs** (GitHub Actions via `GITHUB_PAT`; degrades gracefully if absent). Missing required sandbox templates are critical because spawning from them fails; a missing hosted orchestrator image is a warning because the running service remains usable but its next recreate is unsafe. The orchestrator repair action must use `POST /api/orchestrator/build/stream`, never the sandbox-template rebuild chain. This is the start of Stage 3 (observability) in [MASTER_PLAN.md](MASTER_PLAN.md).
- **Hosted-poller freshness:** never treat the age of `matrx-sandbox.last-deployed-sha` as the age of a newly approved release. While deploy-hosted build markers are fresh, report the release as in progress; before its first marker, use the approved commit age for the 30-minute grace window. Only “approved SHA waited past the threshold + no active marker” is stuck.
...
- **Closed 2026-07-07:** ~~generic config-file editor~~ → **`/admin/files`** (file-tree browse/view/edit on the /srv host AND both EC2 boxes over SSM, `.bak` on every save, superadmin). ~~EC2 env editing~~ → **Secrets remote stores** (`ec2:aidream-app` = `/etc/aidream/app.env`, `ec2:sandbox-orchestrator`) with a per-store **"Apply (restart)"** button (compose stores force-recreate — plain restart does NOT re-read env_file; SSM stores restart the systemd unit with a patient health probe; the Manager's own store deliberately has no Apply). ~~repo `git pull`~~ → effectively closed by the auto-deploy pollers (the matrx-sandbox checkout auto-resets to origin/main every 2 min). Also new: **Fleet Health → aidream ops-triage mirror** every 5 min ([server-manager/src/ops_triage.js](server-manager/src/ops_triage.js), env `MATRX_OPS_SUPABASE_URL/_KEY`) with auto-resolve on recovery; two new checks — **aidream-dedicated** (health of sandbox.matrxserver.com, the dedicated aidream server sandbox-attached chat routes to; it once crashlooped 3 days unseen) and **aidream-pipeline** (aidream deploys are NOT test-gated — warns when production runs a commit with red tests; repo = AI-Matrix-Engine/aidream); **restart-aware health** (Manager-initiated restarts show a calm blue "restarting" for ~2.5 min instead of critical); and **"Rebuild orchestrator" now runs the sandbox DB migration runner** between build and recreate.
- **Runtime-truth hardening (2026-08-09):** EC2 microservice deploy/version state (`matrx-files`, `matrx-seo`) is observed from the actual container over SSM (existence, running image + package version, local health); `/opt/.../CURRENT` is only a separately labeled record. Secrets Apply re-runs the exact inspected image and health-gates success; working vs unavailable buttons are visually distinct. Orchestrator freshness compares each running service's reported `source_sha` with its approved/successful release instead of treating a GitHub record or host checkout as runtime proof. Durable rules: [server-manager/FEATURE.md](server-manager/FEATURE.md).
- **Not implemented at all** ([Plan Phases 4-5](UI_REFACTOR_PLAN.md)): backup scheduling, cert observability, host job inventory.
...
- **Package / Implementation Separation — read before wiring any Matrx service's database env.** The package is CAPABLE, the implementation CHOOSES: a `matrx-*` package must be able to run on its own database, and our implementation points every one of them at the SAME Postgres (Matrx Main). A Matrx service gets ONE required set of variables — `SUPABASE_MATRIX_HOST/_PORT/_DATABASE_NAME/_USER/_PASSWORD` (+ `_SSL`; TLS is a deployment input) — and raises without them. **Banned: a second candidate for a connection** (`<PKG>_DATABASE_URL` → `DATABASE_URL` → `MATRX_<PKG>_POSTGRES_*`); pointing a service at a different database is a change of VALUES, never a new variable name. Ship provisions env for every project, so this repo is where a chain would get born — never inject a `DATABASE_URL` into a Matrx package's container. Ship's OWN `DATABASE_URL` (per-instance Drizzle DB) and the host's `POSTGRES_*` (the shared instance Postgres) are a DIFFERENT product's connection and are unaffected. System of record: `/Users/armanisadeghi/code/common-docs/policies/package-vs-implementation.md`.
