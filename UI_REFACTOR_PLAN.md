...
- **Fleet Health is correctly flagging a known critical:** EC2 orchestrator has 0 aidream secrets loaded (hosted has 163) → EC2 aidream-template boxes fail. Known since 2026-05-24; EC2 tier is barely used (~1 sandbox) and its AWS-local path is slim boxes + the EC2 AI Dream `sandbox_host` replica, so this is low-urgency. The primary public `app_server` remains on Coolify. The fix is **EC2-host config (SSM), not a `/srv` change**. Don't soften the alert — that would mask it.
